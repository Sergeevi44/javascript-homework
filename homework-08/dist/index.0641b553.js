...
const onOverlayClick = (e)=>{
    if (!refs.lightBoxEl.classList.contains('is-open')) return;
...
const onLeftArrowPress = (e)=>{
    if (!refs.lightBoxEl.classList.contains('is-open') || e.code != 'ArrowLeft') return;
    for(let i = 0; i < galleryItems.length; i += 1)if (e.target.href === galleryItems[i].original) {
...
const onRightArrowPress = (e)=>{
    if (!refs.lightBoxEl.classList.contains('is-open') || e.code != 'ArrowRight') return;
    for(let i = 0; i < galleryItems.length; i += 1)if (e.target.href === galleryItems[i].original) {
        refs.imageEl.alt = galleryItems[i + 1].description;
        refs.imageEl.src = galleryItems[i + 1].original;
        return e.target.href = galleryItems[i + 1].original;
...
},{}]},["6IXwR","bNKaB"], "bNKaB", "parcelRequiref466")
